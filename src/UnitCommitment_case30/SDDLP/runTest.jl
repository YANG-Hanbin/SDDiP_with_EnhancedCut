...
@everywhere begin
    ...
    include(joinpath(project_root, "src", "UnitCommitment_case30", "SDDLP", "def.jl"))
    include(joinpath(project_root, "src", "UnitCommitment_case30", "SDDLP", "backwardModel.jl"))
    include(joinpath(project_root, "src", "UnitCommitment_case30", "SDDLP", "forwardModel.jl"))
    include(joinpath(project_root, "src", "UnitCommitment_case30", "SDDLP", "LevelSetMethod.jl"))
    include(joinpath(project_root, "src", "UnitCommitment_case30", "SDDLP", "sddip.jl"))
    ...
    T = 6; num = 5; cutSelection = "ELC"; # "LC", "ELC", "SMC"
...
for cut in ["LC", "ELC", "SMC"]
    for num in [3, 5, 10]
        for T in [6, 8, 12] 
            ...
            if cutSelection == "ELC" 
                save("src/UnitCommitment_case30/numericalResults-$case/Periods$T-Real$num/MagnantiWong/sddlpResult-$ℓ-$tightness.jld2", "sddlpResult", sddlpResult)
            else
                save("src/UnitCommitment_case30/numericalResults-$case/Periods$T-Real$num/sddlpResult-$cutSelection-$tightness.jld2", "sddlpResult", sddlpResult)
            end
...
if cutSelection == "ELC" 
    sddlpResult = load("src/UnitCommitment_case30/numericalResults-$case/Periods$T-Real$num/MagnantiWong/sddlpResult-$ℓ-$tightness.jld2")["sddlpResult"][:solHistory]
else
