...
function evaluate_function(x₀::Vector{Float64}, functionCoefficientInfo::FunctionCoefficientInfo; M::Float64 = 1e5, x̂::Vector{Float64} = [3.0, 0.0], surhat = 0, sur_x = 0, Output::Int64 = 0)
    ...
    @variable(model, sur_c, Bin)
    @constraint(model, x_c[1] ≥ 0 + 1.5 * sur_c)
    @constraint(model, x_c[1] ≤ 1.5 + 1.5 * sur_c)
    ...
    @objective(model, Min, sum(sum(functionCoefficientInfo.a[i][j] * z[i,j] for j in 1:length(functionCoefficientInfo.a[i])) + 
                           sum(functionCoefficientInfo.b[i][j] * y[i, j] for j in 1:length(functionCoefficientInfo.a[i])) for i in 1:functionCoefficientInfo.n)
                            + x₀' * (x̂ - x_c) + sur_x * (surhat - sur_c)) 
    optimize!(model)

    # evaluate obj
    f = JuMP.objective_value(model)
    df = x̂ .- JuMP.value.(x_c)
    df = [df..., surhat .- JuMP.value(sur_c)]
...
function add_constraint(function_info::FunctionInfo, model_info::ModelInfo, iter::Int64)
    ...
    @constraint(model_info.model, [k = 1:m], model_info.y .>= 0. )
end
...
function LevelSetMethod(x̂::Vector{Float64}, x₀::Vector{Float64}; μ::Float64 = 0.5, λ::Float64 = 0.1, Output::Int64 = 0, threshold::Float64 = 1e-5, 
                        functionCoefficientInfo::FunctionCoefficientInfo = functionCoefficientInfo, surhat = surhat, sur_x = sur_x)
    ...
    ## trajectory
    evaluate_function_value = evaluate_function(x₀, functionCoefficientInfo, x̂ = x̂, surhat = surhat, sur_x = sur_x)
    function_info = FunctionInfo(   Dict(1 => [x₀...,sur_x]), 
    ...
    while true
        ...
        @constraint(model_nxt, [k in keys(function_info.G)], y1 .>= 0 )
        ...
        ## save the trajectory
        evaluate_function_value = evaluate_function([x_nxt[1]], functionCoefficientInfo, x̂ = x̂, surhat = surhat, sur_x = x_nxt[2])
