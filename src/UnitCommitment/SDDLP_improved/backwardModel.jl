...
backwardModel!(; indexSets::IndexSets = indexSets, 
                            paramDemand::ParamDemand = paramDemand, 
                                paramOPF::ParamOPF = paramOPF, 
                                    ...
                                    θ_bound::Real = 0.0, outputFlag::Int64 = 0, timelimit::Real = 100, mipGap::Float64 = 1e-2
                            )
...
function backwardPass(
    ...
    solCollection::Dict{Any, Any} = solCollection, 
    ...
    sum(λ₁[:sur][g][k] * solCollection[i, t-1, ω].stageSolution[:sur][g][k] for k in keys(solCollection[i, t-1, ω].stageSolution[:sur][g])) 
    for g in indexSets.G
    );
