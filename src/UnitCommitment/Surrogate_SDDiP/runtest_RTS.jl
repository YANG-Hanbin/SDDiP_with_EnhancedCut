...
####################################### Run Experiment ######################################
#############################################################################################
Output_Gap = false; max_iter = 200; ϵ = 1e-4; cutSelection = "LC"; δ = 50.; numScenarios = 2; tightness = true;
        ...
        for num in [3, 5, 10]
            ...
            sddipResult = SDDiP_algorithm(scenarioTree = scenarioTree, 
                                indexSets = indexSets, 
                                    paramDemand = paramDemand, 
                                        paramOPF = paramOPF, 
                                            initialStageDecision = initialStageDecision, numScenarios = numScenarios,
                                            Output_Gap = Output_Gap, max_iter = max_iter, ϵ = ϵ, δ = δ, cutSelection = cutSelection)
            save("src/UnitCommitment/experiment/stage($T)real($num)/SsddipResult_5hr_$cutSelection.jld2", "sddipResult", sddipResult)
