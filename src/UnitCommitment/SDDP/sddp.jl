...
function SDDP_algorithm( ; scenarioTree::ScenarioTree = scenarioTree, 
        ...
        backwardInfoList[t] = backwardModel!(indexSets = indexSets, 
                                                paramDemand = paramDemand, 
                                                    paramOPF = paramOPF, 
                                                        stageRealization = scenarioTree.tree[t], 
                                                                outputFlag = 0, 
                                                                        mipGap = 1e-4, tightness = true, θ_bound = 0.0, timelimit = 20
                                                );
    end 
    ...
    while true
        ...
        gap = round((UB-LB)/UB * 100 ,digits = 2); gapString = string(gap,"%"); push!(sddpResult, [i, LB, OPT, UB, gapString, iter_time, total_Time]); push!(gapList, gap);
        if i == 1
