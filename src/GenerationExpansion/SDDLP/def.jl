...
struct ForwardModelInfo
    ...
    St              :: Vector{VariableRef} ## stage variable, A * Lt is total number of generators built
...
struct BackwardModelInfo
    ...
    x               :: Vector{VariableRef} ## for current state, x is the number of generators
    St              :: Vector{VariableRef} ## stage variable, A * Lt is total number of generators built
    Sc              :: Vector{VariableRef} ## local cooy variable
    y               :: Vector{VariableRef} ## amount of electricity
    θ               :: VariableRef
    # demand          :: Vector{Float64}
    slack           :: VariableRef
...
mutable struct CurrentInfo
    x            :: Dict{Symbol, Any}                   ## record x point
...
struct ModelInfo
    ...
    x_sur :: JuMP.Containers.SparseAxisArray{VariableRef, 2, Tuple{Int64, Int64}}
    y     :: VariableRef
    z     :: VariableRef
end
...
struct LevelSetMethodParam
    ...
    Ŝ             ::Dict{Symbol, Any}                   ## first stage solution
    cutSelection  ::String
    S̃             ::Union{Dict{Symbol, Any}, Nothing}   ## interior point
...
mutable struct StateVar
    var           :: Dict{Symbol, Dict{Int, VariableRef}}                     # var[:s][g], var[:y][:g]
    sur           :: Dict{Int, Dict{Int, Dict{Symbol, Any}}}                  # sur[g][k][:lb], sur[g][k][:ub], sur[g][k][:var] 
    leaf          :: Dict{Int, Vector{Int64}}                                 # leaf[g] = [k1, k2, k3, ...]
end
