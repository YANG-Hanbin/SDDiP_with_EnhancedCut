...
####################################    main function   #####################################
#############################################################################################
max_iter = 100; ϵ = 1e-4; M = 30; Output_Gap = false; tightness = false; TimeLimit = 60*60*2; MaxIter = 100;
T = 3; num = 5;
...
for cutSelection in ["LC", "ELC", "ShrinkageLC"]
    for T in [3, 5]
        for num in [5, 10]
            ...
            save("src/GenerationExpansion/numerical_data/testData_stage($T)_real($num)/sddlp_tight($tightness)_$cutSelection.jld2", "sddipResults", sddipResults)  
        end
    end
end
