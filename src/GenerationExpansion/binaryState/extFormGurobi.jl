...
# sum( sum( scenario_tree[ω][2] * (stageDataList[t].c1' * value.(x[:, t, ω]) + stageDataList[t].c2' * value.(y[:, t, ω]) + stageDataList[t].penalty * value(slack[t, ω])) for t in 1:T ) for ω in 1:W)

# sum( sum( scenario_tree[ω][2] * (stageDataList[t].c1' * value.(x[:, t, ω]) ) for t in 1:T ) for ω in 1:W)

# sum( sum( scenario_tree[ω][2] * (stageDataList[t].penalty * value(slack[t, ω])) for t in 1:T ) for ω in 1:W)
