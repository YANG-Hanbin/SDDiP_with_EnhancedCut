...
function add_generator_constraint(StageProblemData::StageData, model_info::ForwardModelInfo;
  ...
  @constraint(model_info.model, binaryInfo.A * model_info.sum_generator + model_info.x .<= StageProblemData.ū )  ## no more than max num of generators
  ...
  @constraint(model_info.model, StageProblemData.h * StageProblemData.N 
                          * (binaryInfo.A * model_info.sum_generator + model_info.x + StageProblemData.s₀ ) .>= model_info.y )  # no more than capacity
