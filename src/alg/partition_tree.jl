...
function update_partition_tree!(
    ...
    @constraint(
        ...
        sum(ModelList[t].ContVarLeaf[:s][g][k][:lb] * ModelList[t].ContVarLeaf[:s][g][k][:var] for k in keys(ModelList[t].ContVarLeaf[:s][g]))
    );
    @constraint(
        ModelList[t].model, 
        ModelList[t].ContVar[:s][g] ≤ 
        sum(ModelList[t].ContVarLeaf[:s][g][k][:ub] * ModelList[t].ContVarLeaf[:s][g][k][:var] for k in keys(ModelList[t].ContVarLeaf[:s][g]))
    );
