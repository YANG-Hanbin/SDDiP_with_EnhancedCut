...
result_df = DataFrame(
    ...
    T=Int[], 
    num=Int[], 
    gap_under_5_time=Union{Missing, Float64}[],
    gap_under_5_iter=Union{Missing, Int}[],
    gap_under_1_time=Union{Missing, Float64}[],
    gap_under_1_iter=Union{Missing, Int}[],
    gap_under_01_time=Union{Missing, Float64}[],
    gap_under_01_iter=Union{Missing, Int}[]
);
for cut in [:eps, :Mid]
    for T in [6, 8, 12]
        for num in [5, 10]
            try
                ...
                # 找到 gap 第一次小于 5 的位置
                below1_idx = findfirst(<(5.), gap_vals)

                # 初始化默认值
                gap_under_5_iter = missing
                gap_under_5_time = missing

                if below1_idx !== nothing
                    gap_under_5_iter = solHistory.Iter[below1_idx]
                    gap_under_5_time = solHistory.Time[below1_idx]
                end
                ...
                # 初始化默认值
                gap_under_01_iter = missing
                gap_under_01_time = missing

                if below1_idx !== nothing
                    gap_under_01_iter = solHistory.Iter[below1_idx]
                    gap_under_01_time = solHistory.Time[below1_idx]
                end

                # 添加到DataFrame
                push!(result_df, (
                    cut, T, num, 
                    gap_under_5_time, gap_under_5_iter,
                    gap_under_1_time, gap_under_1_iter,
                    gap_under_01_time, gap_under_01_iter
                    )
                );
            catch e
                @warn "Error processing file: $file_path" exception=(e, catch_backtrace())
...
for cut in [:NormalizedCut]
    for T in [6, 8, 12]
        for num in [5, 10]
            try
                file_path = "src/results/case=case30/alg=SDDPL/T=$T/Real=$num/cut=$(cut)__med=IntervalMed__eps=256__ell=0.5__sparsity=dense__run=20251129.jld2"
                # file_path = "src/results/case=case30/alg=SDDPL/T=$T/Real=$num/cut=$(cut)__med=IntervalMed__eps=256__ell=0.5__run=20251128.jld2"
                solHistory = load(file_path)["sddpResults"][:solHistory]
