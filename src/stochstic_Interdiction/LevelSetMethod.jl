...
"""
function FuncInfo_LevelSetMethod(x₀::Vector{Float64};  # this is the dual variable
                                        backward_stage2_Info::Backward_stage2_Info = backward_stage2_Info,
                                            cutSelection::String = cutSelection, 
                                                f_star_value::Union{Float64, Nothing} = f_star_value, 
                                                    stageData::StageData = stageData, 
                                                        x̂::Vector{Float64} =  x̂, x̃::Union{Vector{Float64}, Nothing} =  x̃, # those are the first-stage variables
                                                            ϵ::Float64 = ϵ )

    if cutSelection == "ELC"
        @objective(backward_stage2_Info.model, Min, backward_stage2_Info.π[scenarioData.s] + x₀' * ( x̂ .- backward_stage2_Info.x) );
        optimize!(backward_stage2_Info.model);
        F_solution = ( F = JuMP.objective_value(backward_stage2_Info.model), 
                            ∇F = x̂ .- round.(JuMP.value.(backward_stage2_Info.x), digits = 6) );

        currentInfo  = CurrentInfo(x₀, 
                                    - F_solution.F - x₀' * ( x̃ .-  x̂),
    ...
    elseif cutSelection == "LC"
        @objective(backward_stage2_Info.model, Min, backward_stage2_Info.π[scenarioData.s] - x₀' * backward_stage2_Info.x );
        optimize!(backward_stage2_Info.model);
        F_solution = (F = JuMP.objective_value(backward_stage2_Info.model), 
                                ∇F = - round.(JuMP.value.(backward_stage2_Info.x), digits = 6) );
...
function LevelSetMethod_optimization!( backward_stage2_Info::Backward_stage2_Info, x₀::Vector{Float64}; 
                                        levelSetMethodParam::LevelSetMethodParam = levelSetMethodParam, 
                                        stageData::StageData = stageData, indexSets::IndexSets = indexSets,
                                        ϵ::Float64 = 1e-4
                                        ) 
    ...
    (x̂, cutSelection, x̃, f_star_value) = (levelSetMethodParam.x̂, levelSetMethodParam.cutSelection, levelSetMethodParam.x̃, levelSetMethodParam.f_star_value)
    ...
    ## trajectory
    currentInfo = FuncInfo_LevelSetMethod(x₀, cutSelection = cutSelection, backward_stage2_Info = backward_stage2_Info, f_star_value = f_star_value, stageData = stageData, x̂ = x̂, x̃ = x̃, ϵ = ϵ);
    ...
    @variable(oracleModel, x[indexSets.D]);
    ...
    @variable(nxtModel, x1[indexSets.D]);
    ...
    if cutSelection == "ELC" || cutSelection == "ELCwithoutConstraint"
        cutInfo =  [ - currentInfo.f - currentInfo.x' *  x̃,  
                                                                    currentInfo.x] 
    elseif cutSelection == "LC"
        cutInfo = [ - currentInfo.f - currentInfo.x' *  x̂,  
                                                                    currentInfo.x] 
    elseif cutSelection == "ShrinkageLC"
        cutInfo = [ currentInfo.L_at_x̂ - currentInfo.x' *  x̂,  
                                                                    currentInfo.x] 
    end 
    ...
    while true
        ...
        ## ==================================================== end ============================================== ##
        ## save the trajectory
        currentInfo = FuncInfo_LevelSetMethod(x_nxt, cutSelection = cutSelection, backward_stage2_Info = backward_stage2_Info, f_star_value = f_star_value, stageData = stageData, x̂ = x̂, x̃ = x̃, ϵ = ϵ);
        iter = iter + 1;
